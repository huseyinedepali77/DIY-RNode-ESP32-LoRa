...
void promisc_disable() {
  ...
  bool eeprom_begin() {
    ...
    if (!file) {
      if (file.open(EEPROM_FILE, FILE_O_WRITE)) {
        ...
        file.seek(0); file.write(0x10);
        file.seek(1); file.write(0x12);
        file.seek(2); file.write(0x01);
        file.seek(ADDR_INFO_LOCK); file.write(0x73);
        file.seek(ADDR_CONF_OK); file.write(0x73);
        #endif
        eeprom_flush();
        return true;
      } else {
  ...
  uint8_t eeprom_read(uint32_t mapped_addr) {
      ...
      #if BOARD_MODEL == BOARD_PROMICRO_DIY
      if (mapped_addr == 0 && byte == 0xFF) return 0x10;
      if (mapped_addr == 1 && byte == 0xFF) return 0x12;
      if (mapped_addr == 2 && byte == 0xFF) return 0x01;
      if (mapped_addr == ADDR_INFO_LOCK && byte == 0xFF) return 0x73;
      if (mapped_addr == ADDR_CONF_OK && byte == 0xFF) return 0x73;
      #endif
      return byte;
  }
#endif
...
bool eeprom_model_valid() {
  ...
  #elif BOARD_MODEL == BOARD_PROMICRO_DIY
  if (model == MODEL_52 || model == MODEL_12 || model == MODEL_FE || model == MODEL_FF) {
	#elif BOARD_MODEL == BOARD_HUZZAH32
	if (model == MODEL_FF) {
	#elif BOARD_MODEL == BOARD_GENERIC_ESP32
	if (model == MODEL_FF || model == MODEL_FE) {
...
bool eeprom_checksum_valid() {
	#if BOARD_MODEL == BOARD_PROMICRO_DIY || defined(BYPASS_EEPROM_CHECK)
	return true;
	#endif
...
void eeprom_conf_load() {
	...
	if (lora_freq < 100000000 || lora_freq > 1000000000) {
		lora_sf = 9;
		lora_cr = 5;
		lora_txp = 14;
		lora_freq = 433325000;
