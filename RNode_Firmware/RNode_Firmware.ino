...
void setup() {
    ...
    Serial.setRxBufferSize(CONFIG_UART_BUFFER_SIZE);
      ...
      delay(200);
    #elif BOARD_MODEL == BOARD_PROMICRO_DIY
      delay(100);
      pinMode(13, OUTPUT); // P0.13 (3V3_EN / EXT_VCC)
  ...
  #if BOARD_MODEL != BOARD_RAK4631 && BOARD_MODEL != BOARD_HELTEC_T114 && BOARD_MODEL != BOARD_TECHO && BOARD_MODEL != BOARD_T3S3 && BOARD_MODEL != BOARD_TBEAM_S_V1 && BOARD_MODEL != BOARD_HELTEC32_V4 && BOARD_MODEL != BOARD_PROMICRO_DIY
...
bool startRadio() {
  #if BOARD_MODEL == BOARD_PROMICRO_DIY
    hw_ready = true;
  #endif
  update_radio_lock();
...
void validate_status() {
  ...
  if (boot_vector == START_FROM_BOOTLOADER || boot_vector == START_FROM_POWERON) {
    eeprom_update(eeprom_addr(ADDR_PRODUCT), PRODUCT_T32_21);
    eeprom_update(eeprom_addr(ADDR_MODEL), MODEL_B4);
    ...
    eeprom_update(eeprom_addr(ADDR_CONF_SF), 9);
    eeprom_update(eeprom_addr(ADDR_CONF_CR), 5);
    eeprom_update(eeprom_addr(ADDR_CONF_TXP), 14);
    
    uint32_t default_freq = 433325000;
    eeprom_update(eeprom_addr(ADDR_CONF_FREQ)+0, (default_freq >> 24) & 0xFF);
    eeprom_update(eeprom_addr(ADDR_CONF_FREQ)+1, (default_freq >> 16) & 0xFF);
    eeprom_update(eeprom_addr(ADDR_CONF_FREQ)+2, (default_freq >> 8) & 0xFF);
    eeprom_update(eeprom_addr(ADDR_CONF_FREQ)+3, default_freq & 0xFF);

    uint32_t default_bw = 125000;
    eeprom_update(eeprom_addr(ADDR_CONF_BW)+0, (default_bw >> 24) & 0xFF);
    eeprom_update(eeprom_addr(ADDR_CONF_BW)+1, (default_bw >> 16) & 0xFF);
    eeprom_update(eeprom_addr(ADDR_CONF_BW)+2, (default_bw >> 8) & 0xFF);
    eeprom_update(eeprom_addr(ADDR_CONF_BW)+3, default_bw & 0xFF);

    eeprom_update(eeprom_addr(ADDR_CONF_DROT), 0);
    eeprom_update(eeprom_addr(ADDR_CONF_DINT), 255);

    uint32_t creation_timestamp = 1700000000;
    eeprom_update(eeprom_addr(ADDR_MADE)+0, (creation_timestamp >> 24) & 0xFF);
    eeprom_update(eeprom_addr(ADDR_MADE)+1, (creation_timestamp >> 16) & 0xFF);
    eeprom_update(eeprom_addr(ADDR_MADE)+2, (creation_timestamp >> 8) & 0xFF);
    eeprom_update(eeprom_addr(ADDR_MADE)+3, creation_timestamp & 0xFF);

    char *chk_data = (char*)malloc(CHECKSUMMED_SIZE);
    for (uint8_t i = 0; i < CHECKSUMMED_SIZE; i++) {
      chk_data[i] = EEPROM.read(eeprom_addr(i));
    }
    unsigned char *chk_hash = MD5::make_hash(chk_data, CHECKSUMMED_SIZE);
    for (uint8_t i = 0; i < 16; i++) {
      eeprom_update(eeprom_addr(ADDR_CHKSUM+i), chk_hash[i]);
    }
    free(chk_hash);
    free(chk_data);
