...
void sx126x::enableTCXO() {
    ...
    #elif BOARD_MODEL == BOARD_RAK4631 || BOARD_MODEL == BOARD_HELTEC32_V3 || BOARD_MODEL == BOARD_XIAO_S3
...
void sx126x::setTxPower(int level, int outputPin) {
  ...
  uint8_t pa_buf[4];
  if (level <= 14) {
    pa_buf[0] = 0x02; // PADutyCycle for <= 14dBm
    pa_buf[1] = 0x02; // HPMax for <= 14dBm
  } else if (level <= 17) {
    pa_buf[0] = 0x02; // PADutyCycle for <= 17dBm
    pa_buf[1] = 0x03; // HPMax for <= 17dBm
  } else if (level <= 20) {
    pa_buf[0] = 0x03; // PADutyCycle for <= 20dBm
    pa_buf[1] = 0x05; // HPMax for <= 20dBm
  } else {
    pa_buf[0] = 0x04; // PADutyCycle for 22dBm
    pa_buf[1] = 0x07; // HPMax for 22dBm
  }
  pa_buf[2] = 0x00; // DeviceSel 0x00 for SX1262
  pa_buf[3] = 0x01; // PALut always 0x01
