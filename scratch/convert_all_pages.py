...
for fname, content in pages.items():
    compiled = str(MarkdownToMicron(content))
    for d in dirs:
        filepath = os.path.join(d, fname)
        with open(filepath, "w", encoding="utf-8") as f:
            f.write(compiled)
    print(f"Compiled {fname} successfully!")
"""

    run_ssh("cat << 'EOF' > /root/convert_pages.py\n" + converter_code + "\nEOF")
    out, err = run_ssh("python3 /root/convert_pages.py")
